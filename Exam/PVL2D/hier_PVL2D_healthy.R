...
win_raw <- ctr_data$gain
loss_raw <- ctr_data$loss
...
X_all <- array(0,c(nsubs,ntrials_max))
win_all <- array(0,c(nsubs,ntrials_max))
...
for (s in 1:nsubs) {
  ...
  win_sub <- win_raw[ctr_data$subjID==subIDs[s]]
  length(win_sub) <- ntrials_max
  
  loss_sub <- loss_raw[ctr_data$subjID==subIDs[s]]
...
data <- list("x","X","win","loss","ntrials","nsubs") 
params<-c("mu_w","mu_A","mu_theta","mu_a","lambda_w","lambda_A","lambda_theta","lambda_a")

start_time = Sys.time()
samples <- jags.parallel(data, inits=NULL, params,
                         model.file ="PVL2D/hier_PVL2D.txt",
                         n.chains=3, n.iter=5000, n.burnin=1000, n.thin=1, n.cluster=4)
end_time = Sys.time()
end_time - start_time

save(samples, file = "PVL2D/PVL2D_jags_fit.RData")
load("PVL2D/PVL2D_jags_fit.RData")
...
acc_plot <- acc_df %>% 
  ...
  xlab("Subject") +
  ylab("Accuracy") +
  ggtitle("PVL2D Predictive accuracies") +
  theme_minimal()
acc_plot
